...
pub fn part2(input: &str) -> Result<()> {
    ...
    for _i in 1..=num_days {
        ...
        state
            .iter_mut()
            .filter(|(counter, _)| *counter < 0)
            .for_each(|(counter, _)| *counter = 6);
    ...
    println!(
        ...
        state
            .iter()
            .fold(0, |acc, (_, occurences)| acc + occurences)
    );
    Ok(())
}
...
pub fn join_occurences(state: Vec<(i64, i64)>) -> Vec<(i64, i64)> {
    ...
    for i in 0..=8 {
        let occurrences = state
            .iter()
            .filter_map(|(timer, occurences)| if *timer == i { Some(occurences) } else { None })
            .sum::<i64>();
        if occurrences > 0 {
